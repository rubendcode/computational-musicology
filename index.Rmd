...
# Visualize the comparison of standard deviation of loudness, tempo, and duration
ggplot(summary_comparison, aes(x = playlist_name)) +
  geom_bar(aes(y = sd_loudness, fill = "Loudness"), stat = "identity", alpha = 0.7) +
  geom_errorbar(aes(ymin = mean_loudness - sd_loudness, ymax = mean_loudness + sd_loudness), width = 0.2) +
  geom_bar(aes(y = sd_tempo, fill = "Tempo"), stat = "identity", alpha = 0.7) +
  geom_errorbar(aes(ymin = mean_tempo - sd_tempo, ymax = mean_tempo + sd_tempo), width = 0.2) +
  geom_bar(aes(y = sd_duration, fill = "Duration"), stat = "identity", alpha = 0.7) +
  geom_errorbar(aes(ymin = mean_duration - sd_duration, ymax = mean_duration + sd_duration), width = 0.2) +
  labs(
    x = "Playlist",
    y = "Standard Deviation",
    title = "Comparison of Standard Deviation for Loudness, Tempo, and Duration"
  ) +
  scale_fill_manual(values = c("Loudness" = "skyblue", "Tempo" = "salmon", "Duration" = "lightgreen")) +
  theme_minimal() +
